# Contributing to `pedal_communication`
All contributions, bug reports, bug fixes, documentation improvements, enhancements, and ideas are welcome.
We recommend going through the list of [`issues`](https://github.com/s2mLab/pedal_communication/issues) to find issues that interest you, preferable those tagged with `good first issue`.
You can then get your development environment setup with the following instructions.

## Forking `pedal_communication`

You will need your own fork to work on the code.
Go to the [pedal_communication project page](https://github.com/s2mLab/pedal_communication/) and hit the `Fork` button.
You will want to clone your fork to your machine:

```bash
git clone https://github.com/your-user-name/pedal_communication.git
```
...
- `cd` to the `pedal_communication` source directory
- Install `pedal_communication` dependencies with:

```bash
conda create -n pedal_communication
conda activate pedal_communication
conda install pip -cconda-forge
pip install . && pip uninstall -y pedal_communication
```
...
The `pedal_communication` test suite runs automatically on GitHub every time a commit is submitted.
...
`pedal_communication` tries to follow as much as possible the PEP recommendations (https://www.python.org/dev/peps/). 
...
`pedal_communication` is linted with the 120-character max per line's option. 
